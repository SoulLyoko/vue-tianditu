  ...
  <div class="tdt-search">
    <div class="search-wrapper">
      <tdt-input
        ...
        @change="search()"
      ></tdt-input>
      <tdt-button size="small" @click="search()">搜索</tdt-button>
      <tdt-button size="small" @click="clearAll();showRoute=true">导航</tdt-button>
    </div>
      ...
      <div class="route-content">
        <p>
          <tdt-input
            ...
            style="width:120px;"
          ></tdt-input>
          <tdt-button type="text" size="mini" @click="startTool.clear();startTool.open()">设起点</tdt-button>
        </p>
        <p>
          <tdt-input
            size="mini"
    ...
    <div class="search-result" v-if="showSearchResult">
      ...
      <div class="prompts" v-for="prompt in prompts">
        ...
        <div v-else-if="prompt.type===3">
          有以下相关结果，您是否要找：
          <p v-for="admin in prompt.admins">
            <a>{{admin.name}}</a>
          </p>
        </div>
      </div>
      <div class="pois" v-if="pois.length">
        <div class="pois-item" v-for="(poi,index) in pois" @click="showPosition(poi)">
          <div>{{ poi.name }}</div>
      ...
      <div class="statistics" v-if="statistics">
        在中国以下城市有结果
        <ul>
          <li v-for="city in statistics.priorityCitys">
...
import TdtButton from "../common-components/tdt-button";
import TdtInput from "../common-components/tdt-input";

export default {
  name: "tdt-search",
  mixins: [componentMixin, searchMixin, routeMixin, suggestMixin],
  components: { TdtButton, TdtInput },
  props: {
