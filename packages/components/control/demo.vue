<template>
  <button class="btn" @click="state.visible = !state.visible">显示自定义控件:{{ state.visible }}</button>
  <button class="btn" @click="state.copyright = !state.copyright">显示默认版权控件:{{ state.copyright }}</button>
...
const state = reactive({
  ...
  controls: [
    ...
    },
    {
      name: "Copyright",
      id: "custom",
      content: `<div><button>自定义的版权控件</button></div>`,
      position: "bottomleft",
      bounds: [
        [113.52791, 23.21989],
        [113.03352, 23.03045]
    ...
    "Scale"
  ] as (VT.ControlName | VT.ControlOptions)[],
  visible: true,
  copyright: true
});
</script>
...
.hide-copyright .tdt-control-copyright.tdt-control > div:not(.tdt-control-copyright) {
  display: none;
}
</style>
